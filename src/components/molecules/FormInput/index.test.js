...
describe('[FormInput]', () => {
  const getInputConfig = config => {
    ...
    const testFormInput = screen.getByTestId(`test-form-${config.inputConfig.type}`)
    expect(testFormInput).toBeInTheDocument()

    cleanup()
    render(<FormInput {...{ ...config, inputConfig: { ...config.inputConfig, isValid: false } }} />)
    const testInvalidFormInput = screen.getByTestId(`test-form-${config.inputConfig.type}`)
